...
def main():
    ...
    for conll in os.listdir(parent_dir):
        ...
        dev = [8, 21, 26, 33, 46, 50, 54, 57, 63, 78, 83, 89, 90, 110, 125, 130, 136, 158, 159, 177]
        with open(parent_dir+conll,"r",encoding="utf-8") as c:
            ...
            for e in conll2:
                if '#end' not in e:
                    ...
                    if int(i) in dev:
                        if len(str(cpt_parts))==2:
                            out_conll='dev/eng_00'+str(cpt_parts)+'.9v_gold_conll'
                            cpt_parts+=1
                        else:
                            out_conll='dev/eng_0'+str(cpt_parts)+'.9v_gold_conll'
                            cpt_parts+=1
                    elif int(i) in test:
                        if len(str(cpt_parts))==2:
